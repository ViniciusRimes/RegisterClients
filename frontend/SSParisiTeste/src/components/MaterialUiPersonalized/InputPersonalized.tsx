...
const InputPersonalized: React.FC<InputProps> = ({htmlFor, text,  id, handleOnChange, value, placeholder, required, maxLength}) => {
  return (
    <FormControl sx={{width: 'calc(50% - 2em)', margin: '0.5em'}}>
        ...
        <TextField size="small" inputProps={maxLength ? {maxLength: maxLength} : undefined} name={htmlFor} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>handleOnChange(e)} value={value} required= {required} placeholder= {placeholder} id={id} variant='outlined' sx={{ margin: '0.5em 0em',fontFamily: 'Poppins', '& input::placeholder': {
            fontFamily: 'Poppins',
            opacity: 0.7,
            fontSize: '0.8em'
        }}}/>
